...
router.post('/', function(req, res, next) {
    ...
    if (user.roomCode != null){
        ...
        return res.status(403).json({message: "Must leave current room before creating another"})
    }
...
router.post('/join', function(req, res, next) {
    ...
    if (user.roomCode != null){
        // TODO: should this ever happen, frontend can prevent this??
        return res.status(403).json({message: "Must leave current room before joining another"})
    }
    let room;
    try {
...
router.patch('/leave', function(req, res, next) {
    ...
    if (user.roomCode == null){
        ...
        return res.status(403).json({message: "User is not currently a member of any room"})
    }
    let room;
    try {
        await Room.updateOne(
...
router.delete('/', function(req, res, next) {
    ...
    if (room.host != user){
        return res.status(403).json({message: "User is not the owner of the room they belong to"});
    }
